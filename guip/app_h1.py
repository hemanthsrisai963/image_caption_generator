...
def dashboard_1():
    ...
    if request.method == 'POST':
            ...
            if file:
                ...
                session['gentext']=generated_text_prefix
...
def process_text():
    ...
    generated_caption = session['gentext']
    input_text = request.form['query']

    # Concatenate generated caption and input text
    input_prompt = generated_caption + " " + input_text

    # Use GPT to generate output
    response = openai.Completion.create(
        engine="text-davinci-003",  # Change to the appropriate engine
        prompt=input_prompt,
        max_tokens=150
    )
    ...
    return render_template('result_1.html', output_text=output_text)
@app.route('/logout')
def logout():
